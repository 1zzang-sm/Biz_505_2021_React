 ...
 * 
 * HTML에서 <input value="b_name" name="b_name"> 이라는 코드는
 ...
 * read only가 되버리면 input을 사용하는 의미가 없어진다.
...
const Write =()=>{
	...
	// setB_Name이라는 함수를 사용하겠다. 라는 선언
	const [b_name, setB_Name] = useState('이몽룡')

	// event객체를 매개변수로 갖는 handler 함수
	/**
	 ...
	 * setB_Name() 함수를 사용하여 b_name변수에 값을 세팅한다.
	 * 
	 * React 내부 rendering되는 알고리즘
	 * setB_Name() 함수에 문자열이 전달되면
	 * 내부에서는
	 * 1. 전달된 문자열을 임시 변수에 할당한다.
	 * 		const _b_name = 전달된 문자열
	 * 2. 실제로 b_name 변수가 가리키고 있는 메모리에 주소를 
	...
	const onWriterChangeHandle = (e)=>{
		...
		// react에서는 상태(변수, 객체)에 직접 값을 할당해서는 절대 안된다.
