...
// JS code에서 전반적으로 사용되는 날짜, 시간을 관리하는 객체
...
function RemList() {
	 ...
	 * 대한민국
	 * 
	 * 위의 예제는 함수를 1번만들고 3번활용(사용)한 것이다.
	 * 
	 * 일반적인 함수는 사용하기 전에 만들고, 사용을 계속하고 .. 이런식으로 진행이 되는데
	 * 
	...
	// 상태가 없으면 최초 rendering될때 한번만 함수를 호출한다
	// 변화를 감시할 상태 데이터가 없으면
	...
	const saveStorage = ()=>{
		 ...
		 * JSON String 문자열로 변환하여 JSON.stringify() 이용해서
		...
		if(rememberList.length > 0){
			 ...
			 * return 1 : next가 앞으로, pre가 뒤로
			 * return이 -1 : pre가 앞으로
			 * return이 0이면 그대로 있어
			 * 
			 * map(), filter()는 결과를 return하여 다른 배열을 생성한다.
			 * sort()는 자기 자신을 변경한다
			 */
			rememberList.sort( (pre, next) =>{
				if(pre.r_comp && !next.r_comp) return -1
				if(!pre.r_comp && next.r_copm) return 1
				if(next.r_date > pre.r_date && next.r_time > pre.r_time) return 1
				// return pre.r_comp 
				// ? 1 
				// : pre.r_date > next.r_date && pre.r_time > next.r_time ? 1
				// : -1;
			})
	...
	// 상태가 없으면 최초의 rendering될때 한번만 함수를 호출한다.
	/**
	 * list중 한 요소를 더블클릭하면 
	 * 선택된 요소의 UUID값을 추출하여
	 * 1. 해당 요소를 삭제하기
	 ...
	 * 
	 * 데이터를 어딘가에(rememberList 상태) 추가하기
	 */
	const onKeyDown = (e)=>{		 
		// 13은 Enter Key code
		// 키보드로 입력하는 도중 Enter키를 누르면
		if(e.keyCode === 13) {
			// 현재까지 입력된 문자열들을 추출한다
			// 이때 문자열은 input box에 value속성에 담겨있다.
			// const value = e.target.value
			const {value} = e.target

			//alert("Enter" + value)
			// input box에 입력된 문자열을 rememberList에 담기위해서
			// 객체로 생성한다
			const remember = {
				r_id : UUID(),
				// moment를 사용하여 현재 시스템의 날짜와 시간 문자열로 가져오기
			 ...
			 * 
			 * const arCopyAdd = [...ar, 9, 10] 기존 배열을 복제하면서 요소를 추가하기
			 * 기존에 rememberList를 새로운 배열로 대치
			 */
			// 생성된 remember 객체 데이터를 rememberList 상태에 추가하기
			// 배열에 요소를 추가하기 1번째 코드 전개연산자 2번째코드 concat
